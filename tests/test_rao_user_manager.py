...
class TestUserManager:
    ...
    @pytest.mark.parametrize(
        "function, par, user, propic, code, cache",
        [
    ...
    def test_get_user_by(
        self, mock_get, mock_user_bfj, function, par, user, propic, code, cache
    ):
        mock_get.reset_mock(side_effect=True)
        mock_get.return_value = MockResponse(
            code=code,
    ...
    def test_get_user_by_id_error(
        self, mock_get, mock_user_bfj, function, par, exception
    ):
        mock_get.reset_mock(side_effect=True)
        mock_get.side_effect = exception()
    ...
    def test_create_user(self, mock_post, data, propic, code, message):
        ...
        mock_post.return_value = MockResponse(
            code=code, json={"user": data, "profile_picture": "", "message": message}
        )
        with mock.patch("mib.rao.utils.Utils.save_profile_picture"):
            with mock.patch.object(
                FileStorage, "save", autospec=True, return_value=None
            ):
                image_name = "test.png"
    ...
    def test_create_user_error(self, mock_post, exception):
        ...
        code, message, user = UserManager.create_user(
            {"birthdate": datetime.strptime("01/01/2021", "%d/%m/%Y")}
        )
        assert code == 500
    ...
    def test_update_user_error(self, mock_put, exception):
        ...
        code, message = UserManager.update_user(
            {"birthdate": datetime.strptime("01/01/2021", "%d/%m/%Y")}, 1
        )
        assert code == 500
        assert message == "Unexpected response from user microservice"

    @pytest.mark.parametrize(
        "code, message", [(202, "user deleted"), (404, "user not found")]
    )
    def test_delete_user(self, mock_del, code, message):
        with mock.patch("mib.rao.user_manager.logout_user", return_value=True):
            ...
            mock_del.return_value = MockResponse(code=code, json={"message": message})
    ...
    @pytest.mark.parametrize(
        "code, message", [(200, "content filter toggled"), (404, "user not found")]
    )
    def test_content_filter(self, mock_get, code, message):
        mock_get.reset_mock(side_effect=True)
        mock_get.return_value = MockResponse(code=code, json={"message": message})
    ...
    def test_authenticate_user(self, mock_post, args, code, user):
        mock_post.reset_mock(side_effect=True)
        mock_post.return_value = MockResponse(
            code=code, json={"user": user, "profile_picture": "", "error_message": ""}
        )
        with mock.patch("mib.rao.utils.Utils.save_profile_picture"):
            if code in [200, 401]:
    ...
    def test_blacklist_add(self, mock_put, args, code, message):
        mock_put.reset_mock(side_effect=True)
        mock_put.return_value = MockResponse(code=code, json={"message": message})
    ...
    def test_user_status(self, mock_get, reported, blocked):
        mock_get.reset_mock(side_effect=True)
        mock_get.return_value = MockResponse(
            code=200, json={"reported": reported, "blocked": blocked}
        )
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                ...
                {
                    "users": [
                        {"id": 1, "nickname": "nick1", "email": "email@email.com"},
                        {"id": 2, "nickname": None, "email": "email1@email1.com"},
                    ]
                },
                [(1, "nick1"), (2, "email1@email1.com")],
            ),
            (404, {}, []),
        ],
    )
    def test_get_recipients(self, mock_get, code, json, res):
        mock_get.reset_mock(side_effect=True)
        mock_get.return_value = MockResponse(code=code, json=json)
        recipients = UserManager.get_recipients(1)
        assert recipients == res

    @pytest.mark.parametrize("exc", [requests.ConnectionError, requests.ConnectTimeout])
