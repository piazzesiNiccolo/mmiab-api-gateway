...
class MessageManager:
    ...
    @classmethod
    def retrieve_received_messages(
        cls, id_usr: int, data: datetime = None
        ...
        try:
            if data is None:
                url = "%s/message/list/received/%s" % (
                    cls.message_endpoint(),
                    str(id_usr),
                )
            else:
                data_format = "y=%d&m=%d&d=%d" % (data.year, data.month, data.day)
                url = "%s/message/list/received/%s?%s" % (
                    cls.message_endpoint(),
                    str(id_usr),
                    data_format,
                )
    ...
    def get_replying_info(cls, id_message: int, id_user: int):
        ...
        if id_message is not None:
            try:
                url = "%s/message/replying_info/%s/%s" % (
                    cls.message_endpoint(),
                    str(id_message),
                    str(id_user),
                )
