...
NGINX_LABEL = os.getenv("NGINX_LABEL", default="mib.nginx_lb")
WORKER_LABEL = os.getenv("WORKER_LABEL", default="mib.api_gateway_worker")
...
def signal_handler(_signo, _stack_frame):
    ...
    """
    logger.info(
        "Received a signal %s, stopping orchestrator..." % signal.Signals(_signo).name
    )
    sys.exit(0)
...
def reload_nginx(client, api_gateways):
    ...
    nginx_containers = client.containers.list(
        filters={"label": NGINX_LABEL, "status": "running"}
    )
...
def render_template(ips):
    ...
    context = {"upstreams": [dict(ip=ip, port=WORKER_PORT) for ip in ips]}
...
def get_currently_running_web_servers(client):
    web_containers = client.containers.list(
        filters={"label": WORKER_LABEL, "status": "running"}
    )
...
def listen_for_events(client, web_servers):
    ...
    for event in client.events(filters=event_filters, decode=True):
        if event["status"] == "start":
            ...
            continue
        logger.info(
            "Detected container {} with {} status".format(event["id"], event["status"])
        )
        reload_nginx(client, web_servers)
